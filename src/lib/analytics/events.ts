...
import { SENSITIVE_KEY_PATTERNS } from './constants';
import { UniversalEvent, TrackPayload } from './types';
...
export type EventName = typeof EVENT_REGISTRY[keyof typeof EVENT_REGISTRY] | string;

/**
 * Strips sensitive keys (passwords, tokens, authorization secrets) from metadata
 */
export function sanitizeMetadata(data: any): Record<string, any> {
 ...
 */
export function buildUniversalEvent(
  eventName: string,
